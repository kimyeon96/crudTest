...
public class UserServiceimpl implements UserService {
    ...
    public List<UserDto> readAllUser() throws Exception {
        ...
        /*userMapper.readAllUser(); // ☆☆ 이걸 호출하면 (반짝이면 호출이 된거)
        //매퍼가 실행이 된거. list형식으로 받아온거
        // List형식의 userDto가 찍혀온 것*/
    ...
    public /*(공개 여부)*/ RedirectDto /*리턴값형태(무슨 형태로 반환할것인가)*/ updateUser /*(변수명/함수명)*/ (UpdateUserDto updateUserDto) /*(파라미터)*/ throws Exception {
        ...
        // 컨트롤 알트 V
        RedirectDto redirectDto = new RedirectDto();
        //for문에서도 쓸거고 if문에서도 쓸거라 위에 이렇게 선언해둔것 리다이렉트는
